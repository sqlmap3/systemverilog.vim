...
" Struct member access: variable.member (but not function calls)
" Pattern: \.\h\w* matches dot followed by identifier (\h=head char, \w*=word chars)
" Negative lookahead: \%(\_s*(\)\@! ensures no '(' follows (\_s* allows whitespace/newlines)
" This distinguishes struct members from object method calls like .size()
syntax match svStructMember "\.\h\w*\%(\_s*(\)\@!"
...
highlight! default link svStructMember Identifier
